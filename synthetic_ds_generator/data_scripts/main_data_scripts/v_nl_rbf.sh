...
cmd3=" nonlinear_proba=0.425,0.56875,0.7125,0.85625,1"
# sometimes we have to catch the rbf outside of the interpolation range( which we set to 20)
cmd4=' nl_mode="rbf" generator.lagged.nl_sampler.limit_startx="[-20.0, 20.0]"  generator.instant.nl_sampler.limit_startx="[-20.0, 20.0]"' 
...
for spec in "${specs[@]}"; do
    if [[ "$spec" == *"big"* ]]; then
        cmd5=" n_vars=7 generator.lagged.max_lags=4" 
    fi 
    if [[ "$spec" == *"small"* ]]; then
        cmd5=" n_vars=5 generator.lagged.max_lags=3" 
    fi


    echo $cmd$spec$cmd2$cmd3$cmd4$cmd5$output_dir_param
    eval $cmd$spec$cmd2$cmd3$cmd4$cmd5$output_dir_param 
